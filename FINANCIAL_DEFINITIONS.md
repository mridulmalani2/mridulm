...
| Operating NWC (import, D2) | `(CurrentAssets − cash − ST investments) − (CurrentLiabilities − current debt − current finance leases)` | Replaces the old `CA − CL` figure, which embeds cash and current debt (a financing answer, not an operating one — Apple's is negative mostly for financing reasons). ONE definition feeds both the historical NWC% and the days method, so the two suggestions can never contradict. | `mapXbrl.ts` (`derived:OperatingNWC`) | `edgar-days`, `edgar-map` |
| Working-capital days (import, D2) | 365 basis: DSO = AR/revenue×365; DIO = inventory/COGS×365; DPO = AP/purchases×365, **purchases = COGS + ΔInventory** when a consecutive prior-year inventory exists (else COGS, noted) | Gated, never silently suggested: financial SIC (6000–6999) ⇒ no days; clean-AP concepts only — a bundled payables+accrued concept makes DPO a **gap** (% fallback), never a bundle-derived number; services filer without inventory ⇒ DIO omitted with a note (NOT a Build-blocking gap); implausible outputs (DSO/DPO > 180, DIO > 365) are flagged; COGS excluding-D&A variants accepted but flagged. | `mapXbrl.ts` (`AR/INVENTORY/AP/COGS_TAGS`, `days_notes`) | `edgar-days` |
| Multi-year history (import, D1) | per-metric annual series: per-period alias resolution, restatement dedup (latest `filed` wins, >1% noted), END-date keying, 350–380d full-year rule, true-span CAGRs, ≥3-point suggestion gate | fy/fp metadata describes the FILING, not the fact's period — never keyed on. Derived series (EBITDA) exist only where every component resolves (no fake totals). | `history.ts`, `mapXbrl.ts` (`RawHistoricals.history`) | `edgar-history` |
...
| Missing factual field | a field NO source/layer can derive → provenance `'missing'` | Rendered as an EMPTY input with a red **MISSING** badge; a neutral placeholder is kept in `ModelState` only so the engine solve doesn't divide-by-zero (never shown). NEVER a guessed default, NEVER tagged `'user'`. Editing it flips the source to `'user'`. | `buildModel.ts` (`missingProv`), `AssumptionsReview.tsx`, `ProvenanceBadge.tsx` | `edgar-missing`, `edgar-screens-smoke` |
| Derived-figure taint | a displayed figure computed from a `'missing'` placeholder shows **N/C**, never the fabricated number | The invariant is "no value DERIVED from an unconfirmed input is presented as real" — not just "the input box is empty". A single shared dependency map (`lib/edgar/missing.ts` `dependsOnMissing`) gates every surface: EV / entry multiple depend on revenue + margin only; implied returns (entry equity, IRR, MOIC) are N/C if ANY factual input is missing; the AI-suggest prompt omits any missing fact (never sends a placeholder as an "extracted fact"); Build stays blocked until all are filled. Unknown output paths fail safe (tainted if anything is missing). | `lib/edgar/missing.ts`, `AssumptionsReview.tsx`, `store/dealEngine.ts` (`aiSuggestAssumptions`, `buildModelFromDraft`) | `edgar-taint`, `edgar-screens-smoke`, `edgar-missing` |
