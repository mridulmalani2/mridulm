...
const Hobbies: React.FC = () => {
  ...
  const openAlbum = (album: Album) => {
    ...
    // Scroll to the top of the hobbies section for a fresh view
    ...
    <div id="hobbies" className="min-h-screen py-32 px-6 bg-[#050505] text-[#f4ead5] relative transition-all duration-700">
      <div className="max-w-7xl mx-auto">
        <AnimatePresence mode="wait">
          {!selectedAlbum ? (
            /* ALBUM STACKS VIEW (Chapter V Intro) */
            <motion.div
              key="album-list"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              ...
              <div className="mb-24">
                <motion.div
                  ...
                  className="space-y-4"
                >
                  <h2 className="font-montserrat text-amber-500/50 tracking-[0.5em] text-xs font-bold uppercase">FIND OUT MORE</h2>
                  <h3 className="font-playfair text-5xl md:text-8xl italic leading-none tracking-tighter">A Tale Through <br/>Pictures</h3>
                </motion.div>
              </div>
                    ...
                    <div className="relative w-full aspect-[4/5] mb-8">
                      ...
                      <div className="relative h-full w-full overflow-hidden rounded-2xl border border-white/10 bg-neutral-900 shadow-2xl transition-transform duration-700 group-hover:scale-[1.03]">
                        ...
                        <div className="absolute bottom-6 left-6 right-6">
                           <p className="font-montserrat text-[9px] tracking-[0.3em] text-amber-500 uppercase font-bold mb-1">{album.images.length} CAPTURES</p>
            ...
            /* EXPANDED GRID VIEW (Easy Cards format) */
            <motion.div
              key="album-expanded"
            ...
            >
              ...
              <div className="mb-20 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
                <div className="space-y-4">
                  <motion.button
                    onClick={closeAlbum}
                    className="flex items-center gap-3 group mb-6 text-amber-500/80 hover:text-amber-500 transition-colors"
                  >
                    <ChevronLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
                    <span className="font-montserrat text-[10px] font-bold tracking-[0.4em] uppercase">BACK TO ALBUMS</span>
                  </motion.button>
                  <h3 className="font-playfair text-5xl md:text-7xl italic leading-none">{selectedAlbum.name}</h3>
                  <p className="font-montserrat text-sm md:text-xl text-white/50 max-w-2xl font-light italic tracking-wide">
                    "{selectedAlbum.footerText}"
                  </p>
                </div>
                <div className="hidden md:block">
                  <div className="px-4 py-2 border border-white/10 rounded-full bg-white/5">
                    <span className="font-montserrat text-[10px] tracking-[0.2em] font-bold text-white/30 uppercase">
              ...
              {/* Easy Cards Grid - Robust and Responsive */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {selectedAlbum.images.map((img, idx) => (
                  ...
                  >
                    <img 
                      src={img} 
                      alt={`${selectedAlbum.name} gallery ${idx}`} 
              ...
              <div className="mt-24 flex justify-center">
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="px-12 py-4 border border-white/20 rounded-full font-montserrat text-[10px] tracking-[0.4em] font-bold text-white/50 hover:text-white hover:border-white transition-all uppercase"
                >
                  Return to Archive
                </motion.button>
              </div>
