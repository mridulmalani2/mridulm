...
import { money, num, pct, staleness } from '../../../lib/format';
...
const HistoryTable: React.FC<{ facts: DealFacts; today?: Date }> = ({ facts, today = new Date() }) => {
        ...
        <tbody>
          {rows.map((h) => (
            <tr key={h.period_end} style={{ borderTop: '1px solid rgba(17,17,17,0.08)', color: '#111' }}>
              ...
              <td className="text-right">{cell(h.revenue, (x) => money(x, ccy, 0))}</td>
              <td className="text-right">{cell(h.ebitda, (x) => money(x, ccy, 0))}</td>
