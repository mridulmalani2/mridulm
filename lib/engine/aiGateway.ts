/** AI Gateway — calls Anthropic Claude API directly from the browser. */
...
export async function callAnthropic(
  ...
  if (!toolInput) {
    return { updatedStateDict: null, analysis: null, appliedDiffs: [], triggerRecalculation: false, intent, error: 'AI did not return a tool_use block' };
  }

  const assumptionUpdates = (toolInput.assumption_updates || {}) as Record<string, unknown>;
  const triggerRecalc = (toolInput.trigger_recalculation as boolean) || false;
  const analysis = (toolInput.analysis || null) as AIAnalysis | null;
  ...
  for (const [path, value] of Object.entries(assumptionUpdates)) {
    try {
  ...
  return {
    ...
    analysis,
    appliedDiffs,
    triggerRecalculation: triggerRecalc,
    intent,
  };
}
