...
/**
 * v1 scope guard: a term tranche maturing INSIDE the hold would balloon with no
 * refinancing module (Phase G); §3 defines no balloon step. Pending the spec amendment
 * (recorded with the post-breach gap), such structures are rejected loudly rather than
 * silently mis-modeled. The revolver's maturity only drives its fee-amortization horizon.
 */
