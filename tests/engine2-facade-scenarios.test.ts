...
describe('facade.ts — assembled ModelOutput mirrors & coherence (C5 gate)', () => {
  ...
  it('coherence flags fire on engineered defects (§16 — each code reachable, right severity)', () => {
    ...
    // floor breach: FINAL-year capex hole deeper than the revolver (post-breach
    // continuation semantics land with the v1.0.3 amendment — a mid-hold breach
    // currently rejects the following year's negative opening cash by design)
    const breach = runModel(facts, {
      ...assumptions,
      operations: { ...assumptions.operations, growth_capex: [0, 0, 0, 0, 40] },
    });
    expect(breach.coherence.some((f) => f.code === 'cash_floor_breach' && f.severity === 'block')).toBe(true);
    expect(breach.waterfall.some((w) => w.cash_floor_breach)).toBe(true);
